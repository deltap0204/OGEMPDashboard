...
const PackageEdit = ({
  ...
  const handleEditPanelChange = async (type) => {
    try {
      ...
      const notiOps = getNotificationOpt('package', 'error', 'update');
      enqueueSnackbar(notiOps.message, notiOps.options);
    }
  };
  ...
  const handleDeleteDialogChange = async (type, value) => {
    try {
      if (type === 'btnClick') {
        if (!checkbox && value) {
          const notiOps = getNotificationOpt('package', 'warning', 'delete');
          enqueueSnackbar(notiOps.message, notiOps.options);
          return;
        }
      ...
      console.log(error.message);
      const notiOps = getNotificationOpt('package', 'error', 'delete');
      enqueueSnackbar(notiOps.message, notiOps.options);
    }
  };
