...
const ArchiveMain = ({
  ...
  const [createGrouping] = useMutation(graphql.mutations.createGrouping, {
    update(cache, { data: { createGrouping } }) {
      ...
      setShowEdit(true);
      history.push(`/archives/${createGrouping._id}`);
    }
  });
    ...
    try {
      ...
      if (type === 'btnClick') {
        if (value) {
          ...
          const notiOps = getNotificationOpt('archive', 'success', 'create');
      ...
      const notiOps = getNotificationOpt('archive', 'error', 'create');
      enqueueSnackbar(notiOps.message, notiOps.options);
    }
  };
    ...
    >
      ...
      >
        <CustomInput
          ...
          helperText="The name must be more than 4 character"
          variant="outlined"
          width="300px"
        />
      </CustomDialog>
