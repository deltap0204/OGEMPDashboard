...
import DistrictMain from './Main';
import DistrictEdit from './Edit';
import useStyles from './style';

const SchoolDistrictContainer = ({ history, match }) => {
  ...
  useEffect(() => {
    if (stateContext.district) {
      setShowEdit(true);
      setSelectedDocId(stateContext.district['_id']);
      setEditPanelData(stateContext.district);
    }
  }, []);
  ...
  useEffect(async () => {
    if (context) {
      if (context.groupingAdd) {
        ...
        if (collectionName === 'Topologies' && type === 'district') {
          // setTopologyData({
          //   ...topologyData,
  ...
  const handleMainChange = (type, value) => {
    if (type === 'elSingleClick') {
      ...
      history.push({
        pathname: `/school-districts/${value['_id']}`
      });
      if (!whenState) {
        setShowEdit(true);
...
export default withRouter(SchoolDistrictContainer);
