...
import OERMain from './Main';
import OEREdit from './Edit';
import useStyles from './style';

const OERContainer = ({ history, match }) => {
  ...
  useEffect(() => {
    if (stateContext.oer) {
      setShowEdit(true);
      setSelectedDocId(stateContext.oer['_id']);
      setEditPanelData(stateContext.oer);
    }
  }, []);
  ...
  useEffect(async () => {
    if (context) {
      if (context.groupingAdd) {
        ...
        if (collectionName === 'OER' && type === 'gallery') {
          setShowEdit(true);
  ...
  const handleMainChange = (type, value) => {
    if (type === 'elSingleClick') {
      ...
      history.push({
        pathname: `/galleries/oer/${value['_id']}`
      });
      if (!whenState) {
        setShowEdit(true);
...
export default withRouter(OERContainer);
