...
const PBSMain = ({ selectedDocId, variables, resources, onChange }) => {
  ...
  useEffect(() => {
    setCreateDialogSetting({
      error: false,
      helperText: 'Please enter the new PBS name'
    });
  }, []);
  ...
  const handleCreateDialogChange = async (type, value) => {
    try {
      ...
      setCreateDialogSetting({
        error: true,
        helpText: 'This pbs already exist!'
      });
