...
const SuperAdminContainer = ({ history }) => {
  const classes = useStyles();
  const optionType = 'super';
  const historyKey = 'superAdmins';
  ...
  const handleMainChange = (type, value) => {
    if (type === 'elSingleClick') {
      ...
      setShowEdit(true);
      history.push({
        pathname: `/peoples/super-admins/${value['_id']}`
      });
    }
...
export default withRouter(SuperAdminContainer);
