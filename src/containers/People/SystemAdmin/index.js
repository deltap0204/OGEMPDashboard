...
const SystemAdminContainer = ({ history }) => {
  const classes = useStyles();
  const optionType = 'system';
  const historyKey = 'systemAdmins';
  ...
  const handleMainChange = (type, value) => {
    if (type === 'elSingleClick') {
      ...
      setShowEdit(true);
      history.push({
        pathname: `/peoples/system-admins/${value['_id']}`
      });
    }
...
export default withRouter(SystemAdminContainer);
