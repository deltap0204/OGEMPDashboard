...
const StationAdminContainer = ({ history }) => {
  const classes = useStyles();
  const optionType = 'station';
  const historyKey = 'stationAdmins';
  ...
  const handleMainChange = (type, value) => {
    if (type === 'elSingleClick') {
      ...
      setShowEdit(true);
      history.push({
        pathname: `/peoples/station-admins/${value['_id']}`
      });
    }
...
export default withRouter(StationAdminContainer);
